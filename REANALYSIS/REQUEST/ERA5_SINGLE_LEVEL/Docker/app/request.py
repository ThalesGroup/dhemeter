...
def read_coordinates(metaparams):
    with open(metaparams, 'r') as f:
        ...
        if 'western_lon' in meta_params and 'eastern_lon' in meta_params and 'southern_lat' in meta_params and 'northern_lat' in meta_params:
            print(meta_params['western_lon'], meta_params['eastern_lon'], meta_params['southern_lat'], meta_params['northern_lat'])
...
def Create_ERA5_Single_Level_Request(variables, rea_hours, output_dir, file_name, western_lon, eastern_lon, southern_lat, northern_lat):
    print(rea_hours)
...
def Exec_Logs():
    ...
    execution_dict = {}
    execution_dict['Data_name'] = 'ERA5_SINGLE_LEVEL'
...
def main(params_time, params_vars, output_dir, execution_file, metaparams):
    ...
    with open(params_vars, 'r') as f:
        ...
        if json_file['ERA5_SINGLE_LEVEL']['variables'] == []:
    ...
    with open(params_vars, 'r') as f:
        ...
        dict_variables = params_variables['ERA5_SINGLE_LEVEL']['variables']
        name_variables = []
    ...
    # create the file name with the first hour and the last hour
    first_hour = rea_hours[0]
    last_hour = rea_hours[-1]
    file_name = "ERA5_SINGLE_LEVEL_" + first_hour + "_" + last_hour + ".grib"
    attempts += 1
    # create the request
    if Create_ERA5_Single_Level_Request(name_variables, rea_hours, output_dir, file_name, western_lon, eastern_lon, southern_lat, northern_lat):
