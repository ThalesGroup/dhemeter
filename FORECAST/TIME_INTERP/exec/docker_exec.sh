...
echo docker run -v $data_out_dir:/data/raw -v $data_out_dir:/data/out -v $data_tmp_dir:/data/tmp -v $logs_dir:/data/logs -v $config_dir:/data/config:ro --user $UID:$(id -g) agregateur-forecast-time_interp --logs_file "/data/logs/TIME_INTERP_FORECAST.log" --params_time "/data/config/params_time.json" --output_dir "/data/out" --input_dir "/data/raw" > $logs_dir/FORECAST_TIME_INTERP_out.log 2> $logs_dir/FORECAST_TIME_INTERP_err.log
