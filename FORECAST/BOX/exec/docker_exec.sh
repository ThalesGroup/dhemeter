...
output_file="merge_box_forecast.nc"
...
echo docker run -v $data_out_dir:/data/raw -v $data_out_dir:/data/out -v $data_tmp_dir:/data/tmp -v $logs_dir:/data/logs -v $config_dir:/data/config:ro --user $UID:$(id -g) agregateur-forecast-box --input_file $input_file --input_dir "/data/raw" --output_file $output_file --output_dir "/data/out" --logs_file "/data/logs/BOX_FORECAST.log" --metaparams /data/config/metaparams.json > $logs_dir/BOX_FORECAST_out.log 2> $logs_dir/BOX_FORECAST_err.log
