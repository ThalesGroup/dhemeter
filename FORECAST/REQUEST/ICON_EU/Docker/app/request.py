...
# [ROOT]/[HH]/[var]/icon-eu_europe_regular-lat-lon__[Level_type]_[yyyymmdd][HH]_[FFF]['_'NB_LEV]_[VAR].grib2.bz2	
def Create_URL_ICON_EU (root, run_hour, date, hour, var, level_type, nb_lev):
    # nb lev is optional, if not given, it is not included in the url
    if nb_lev == None and level_type != 'time-invariant':
        url = root + '/' + run_hour + '/' + var + '/icon-eu_europe_regular-lat-lon_' + level_type + '_' + date + run_hour + '_' + hour + '_' + var.upper() +'.grib2.bz2'
    elif nb_lev == None and level_type == 'time-invariant':
        url = root + '/' + run_hour + '/' + var + '/icon-eu_europe_regular-lat-lon_' + level_type + '_' + date + run_hour + '_' + var.upper() +'.grib2.bz2'
    else:
        url = root + '/' + run_hour + '/' + var + '/icon-eu_europe_regular-lat-lon_' + level_type + '_' + date + run_hour + '_' + hour + '_' + str(nb_lev) + '_' + var.upper() +'.grib2.bz2'
    return url

def get_interp_hours(selected_hours, ICON_EU_fc_hours):
    # selected hours is a list of hours for which we want to retrieve data
    # ICON_EU_fc_hours is a list of forecast hours theoretically available for the ICON_EU model
    # if the selected hours are not in the ICON_EU_fc_hours list, we need to select the 2 closest hours lower and upper
    common_hours = []
    for fc_hour in selected_hours:
        if fc_hour in ICON_EU_fc_hours:
            if str(fc_hour) not in common_hours:
                common_hours.append(str(fc_hour).zfill(3))
        else:
            nearest_lower = max(filter(lambda x: x < fc_hour, ICON_EU_fc_hours))
            nearest_upper = min(filter(lambda x: x > fc_hour, ICON_EU_fc_hours))
...
def Exec_Logs():
    ...
    execution_dict = {}
    execution_dict['Data_name'] = 'ICON_EU'
...
def main(params_time, params_variables, output_dir, execution_file, config_file, metaparams):
    ...
    with open(params_variables, 'r') as f:
        ...
        if json_file['ICON_EU']['variables'] == []:
    ...
    with open(metaparams, 'r') as f:
        json_file = json.load(f)
        if not json_file['MODEL']['ICON_EU']:
                ...
                for variable in variables:
                    ...
                    if level_type != 'time-invariant':
                        for level in levels:
                            for hour in hours:
                                try:
                                    url = Create_URL_ICON_EU(root, run_hour, date, hour, variable['variable'], level_type, level)
                                    attempts += 1
