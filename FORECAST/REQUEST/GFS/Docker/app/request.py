...
# function to retrieve data from NOAA NOMADS server based on url path given
def Create_URL_GFS (root, run_hour, date, hour):
    url = root + 'gfs.' + date + '/' + run_hour + '/atmos/gfs' + '.t' + run_hour + 'z.pgrb2.0p25.f' + hour
    return url

def get_interp_hours(selected_hours, GFS_fc_hours):
    # selected hours is a list of hours for which we want to retrieve data
    # GFS_fc_hours is a list of forecast hours theoretically available for the GFS model
    # if the selected hours are not in the GFS_fc_hours list, we need to select the 2 closest hours lower and upper
    common_hours = []
    for fc_hour in selected_hours:
        if fc_hour in GFS_fc_hours:
            if str(fc_hour) not in common_hours:
                common_hours.append(str(fc_hour).zfill(3))
        else:
            nearest_lower = max(filter(lambda x: x < fc_hour, GFS_fc_hours))
            nearest_upper = min(filter(lambda x: x > fc_hour, GFS_fc_hours))
...
def main(params_time, params_variables, output_dir, execution_file, config_file, metaparams):
    ...
    root = 'https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/'
    ...
    with open(params_time, 'r') as f:
        ...
        for hour in hours:
            url = Create_URL_GFS(root, run_hour, date, hour)
            attempts += 1
            try:
                print('start requesting at url:', url)
                r = requests.get(url, allow_redirects=True)
                # if file requested is less than 1000 bytes, it means that the file is not available
                if len(r.content) < 1000:
                    raise Exception('File not available')
                print('Downloaded file : ' + url)
                filename = 'GFS_' + date + '_' + run_hour + '_' + hour + '.grib2'
